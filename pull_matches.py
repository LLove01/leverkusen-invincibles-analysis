...
def download_all_leverkusen_matches(base_path):
    """
    Download all Bayer Leverkusen matches for the 2023/2024 Bundesliga season.
    ...
    try:
        ...
        # Filter matches for Bayer Leverkusen
        leverkusen_matches = matches[
            (matches['home_team'] == 'Bayer Leverkusen') | 
            (matches['away_team'] == 'Bayer Leverkusen')
        ]
        
        print(f"Total Bayer Leverkusen matches found: {len(leverkusen_matches)}")
        
        # Process each match
        for _, match_row in leverkusen_matches.iterrows():
...
def main():
    ...
    download_all_leverkusen_matches(base_path)
